  ...
  <div className="custom-dots" style={{
    ...
    {dots.map((_, index) => {
      const isActive = Array.from({ length: slidesToShow }, (_, i) => {
        ...
        return slideIndex === index || (slideIndex < 0 && slideCount + slideIndex === index);
      }).some(Boolean);
...
const ServicesCards: React.FC = () => {
  ...
  const updateHeight = () => {
    if (sliderRef.current) {
      const slider = sliderRef.current as unknown as { innerSlider?: { list?: HTMLElement } };
          ...
          >
            ...
            >
              ...
              <div className="p-4 md:p-6 pt-3 text-gray-800 flex flex-col flex-grow">
                <h3 className="mb-2 text-lg md:text-xl font-semibold text-center">{service.title}</h3>
                <p className="mb-4 text-sm md:text-base text-center flex-grow">{service.description}</p>
